...
def printlike_multiscale(image):
    paper = make_multiscale_noise_uniform(image.shape, range=(0, 0.2))
    paper = ndi.gaussian_filter(paper, 1)
    return image + paper - mean(paper)
...
def printlike_fibrous(image, blur=1.0, blotches=5e-5):
    ...
    paper = make_multiscale_noise(image.shape, [1.0, 5.0, 10.0, 50.0], weights=[1.0, 0.3, 0.5, 0.3], range=(0.7, 1.0))*random.uniform(0.05, 0.15)
    paper -= make_fibrous_image(image.shape, 300, 500, 0.01, range=(0.0, 0.25), blur=0.5)
    ink = make_multiscale_noise(image.shape, [1.0, 5.0, 10.0, 50.0], range=(0.0, 0.5))*random.uniform(0.05, 0.15)
