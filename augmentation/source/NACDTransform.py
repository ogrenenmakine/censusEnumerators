...
class NACDDefaultTransform(object):
    ...
    """
    def __init__(self, width, height, val, anchors=None, mean=(123.68, 116.779, 103.939),
                 iou_thresh=0.5, box_norm=(0.1, 0.1, 0.2, 0.2),
                 **kwargs):
        self._width = width
        self._height = height
    ...
    def __call__(self, src, bbox):
        ...
        # random cropping
        h, w, _ = img.shape
        bbox, crop = random_crop_with_constraints(bbox, (w, h), self._height, self._width,min_scale=0.9,max_scale=1,max_trial=50)
        x0, y0, w, h = crop
